...
ListingEntry listing1 = ListingEntry(1, 1, "First Listing", "A profession is a type of class commonly found in most RPGs and is central to the gameplay in Guild Wars. Each profession has an array of attributes and skills that help narrow a class's proficiency in order to perform a customized role that is determined by the player. ", "nFTTokenNum", 10000.0);
...
int generatePhotoID(){
  ...
  int maxID= 1;
  for(var i = 0; i < photos.length; ++i){
    if(photos[i].photoID >maxID){
      maxID = photos[i].photoID;
    }
  }
  return maxID + 1;
}
